<template>
    <div class="movie_card" id="bright" @click.prevent="goTo(anime)">
       <style>
        :root {
        --image: url('{{ anime['coverImage']['medium'] }}');
    }
  </style>
        <div class="info_section">
            <div class="movie_header">
                <img class="locandina" :src="anime['coverImage']['medium']"/>
                <h1>{{anime['title']['romaji']}}</h1>
                <!-- <h4>{{anime['startDate']}}, {{anime['countryOfOrigin']}}</h4> -->
                <span class="minutes">{{anime['episodes']}} серии</span>
                <p class="type">{{anime['genres']}}</p>
            </div>
...
export default {
    ...
    methods:{
      goTo(info){
        this.$router.push("/animeInfo/"+info['id']);
      }
    }
}
</script>
